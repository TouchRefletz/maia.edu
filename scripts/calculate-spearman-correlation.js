...
async function run() {
  ...
  Object.entries(questoesDb).forEach(([provaKey, mapQuestoes]) => {
    if (mapQuestoes && typeof mapQuestoes === 'object') {
      Object.entries(mapQuestoes).forEach(([questaoKey, fullData]) => {
        const aiComplexity = calcularComplexidadePct(
  ...
  if (!fs.existsSync(difficultiesPath)) {
    ...
    totalQuestoes.forEach((q) => {
      template[q.id] = {
        ...
        observacoes:
          'Adicione a taxa real de acerto (ex: 0.45 para 45%) ou parâmetro de dificuldade da TRI da banca.',
      };
    });
    ...
    console.log(
      '👉 Por favor, abra esse arquivo JSON e insira os dados de dificuldade real da banca para cada questão.',
    );
    console.log(
      'Depois que preencher, execute este script novamente para rodar o cálculo de correlação de Spearman.',
    );
    process.exit(0);
  }
  ...
  if (matchedList.length < 3) {
    ...
    console.warn(
      "Certifique-se de alterar 'real_difficulty_pct_or_score' para valores diferentes de 0.0 no JSON.",
    );
    process.exit(0);
  }
