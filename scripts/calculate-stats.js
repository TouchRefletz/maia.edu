...
import { calcularIEP, calcularWilcoxonPareado } from '../js/utils/statistics-helper.js';
...
async function run() {
  ...
  if (ungradeds.length > 0) {
    ...
    console.warn('Recomendamos rodar o avaliador com a API correspondente antes.');
    console.warn('Os critérios Likert para estes arquivos serão desconsiderados.');
  }
  ...
  for (const pair of pairs) {
    ...
    if (ctrlEval?.criterios && expEval?.criterios) {
      ...
      if (cJuiz.grupo_f && eJuiz.grupo_f) {
  ...
  const resultados = [
    ...
    testarMetrica('Integração Interdisciplinar (FUVEST)', interdisciplinarI, interdisciplinarII),
  ].filter(Boolean);
  ...
  const iepRes = resultados.find((r) => r.nome === 'Índice de Eficiência de Processamento (IEP)');
  if (iepRes) {
