...
const PDFJS_PATH = path.resolve(__dirname, '../node_modules/pdfjs-dist/build/pdf.js');
const PDFJS_WORKER_PATH = path.resolve(__dirname, '../node_modules/pdfjs-dist/build/pdf.worker.js');
const SHARED_HASH_LOGIC_PATH = path.resolve(__dirname, '../js/utils/shared-hash-logic.js');
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  (async () => {
    ...
    const targetDir = arg;
    if (!targetDir) {
      console.error('Usage: node compute-hash.js <directory_path> OR <pdf_file>');
      process.exit(1);
    }
    ...
    try {
      ...
      for (const item of items) {
        if (item.filename && item.filename.toLowerCase().endsWith('.pdf')) {
          if (targetFiles.length > 0 && !targetFiles.includes(item.filename.toLowerCase())) {
            continue;
          }
