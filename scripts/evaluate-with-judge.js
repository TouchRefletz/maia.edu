...
} from '../js/chat/prompts/judge-prompt.js';
...
async function run() {
  ...
  for (const key of Object.keys(groups)) {
    ...
    if (withRag) {
      ...
      isFuvest = String(fd.vestibular || '')
        .toUpperCase()
        .includes('FUVEST');
    } else {
      enunciado = logGroup[0].data.prompt_original;
      gabarito =
        'Gabarito de referência indisponível. Avalie com base no rigor científico e corretude intrínseca.';
    ...
    for (const log of logGroup) {
      ...
      if (
        ...
        typeof log.data.avaliacao_juiz !== 'object' ||
        log.data.avaliacao_juiz.criterios
      ) {
        log.data.avaliacao_juiz = {};
      }
      ...
      for (const judge of activeJudges) {
        const modelKey =
          judge === 'gemini' ? 'gemini_3_5_flash' : judge === 'gemma' ? 'gemma_4_31b_it' : 'o1';

        if (generatorKey === judge) {
          console.log(
            `  -> Juiz [${modelKey}] foi o modelo gerador da resposta (${log.data.model}). Evitando auto-avaliação.`,
          );
          continue;
        }
        ...
        console.log(
          `  -> Executando avaliação com o juiz: [${modelKey}] para o arquivo ${log.file}...`,
        );
        ...
        try {
          ...
          try {
            ...
            console.warn(
              `  ⚠️ Falha ao converter resposta do juiz [${modelKey}] em JSON. Salvando texto bruto.`,
              pe.message,
            );
