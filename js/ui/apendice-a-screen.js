...
export async function iniciarModoApendiceA() {
        ...
        <div id="containerApendiceASorteio" style="display: block;">
          <div style="margin-bottom: 24px;">
            ...
            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap; margin-top: 12px;">
              <button id="btnAreaLinguagensApendiceA" class="nav-tab-btn active js-area-tab" data-area="Linguagens" style="padding: 10px 18px; border-radius: 6px; font-weight: bold; cursor: pointer; border: 1px solid var(--color-primary); background: var(--color-primary); color: var(--color-btn-primary-text); font-size: 0.9rem; transition: all 0.2s;">
                📖 Linguagens (25 questões)
              </button>
              <button id="btnAreaHumanasApendiceA" class="nav-tab-btn js-area-tab" data-area="Ciências Humanas" style="padding: 10px 18px; border-radius: 6px; font-weight: bold; cursor: pointer; border: 1px solid var(--color-border); background: none; color: var(--color-text); font-size: 0.9rem; transition: all 0.2s;">
                🌍 Ciências Humanas (25 questões)
              </button>
            </div>
          </div>

          <!-- Tabela de Resultados do Sorteio -->
          <div id="apendiceATableArea" style="margin-top: 20px;"></div>
        </div>
  ...
  // Setup Listeners do Sorteio por Abas
  let selectedAreaValue = 'Linguagens';
  const btnLinguagens = document.getElementById('btnAreaLinguagensApendiceA');
  const btnHumanas = document.getElementById('btnAreaHumanasApendiceA');
  const tableArea = document.getElementById('apendiceATableArea');

  const setAreaActiveTab = (area) => {
    selectedAreaValue = area;
    if (area === 'Linguagens') {
      btnLinguagens.style.border = '1px solid var(--color-primary)';
      btnLinguagens.style.background = 'var(--color-primary)';
      btnLinguagens.style.color = 'var(--color-btn-primary-text)';
      btnLinguagens.classList.add('active');

      btnHumanas.style.border = '1px solid var(--color-border)';
      btnHumanas.style.background = 'none';
      btnHumanas.style.color = 'var(--color-text)';
      btnHumanas.classList.remove('active');
    } else {
      btnHumanas.style.border = '1px solid var(--color-primary)';
      btnHumanas.style.background = 'var(--color-primary)';
      btnHumanas.style.color = 'var(--color-btn-primary-text)';
      btnHumanas.classList.add('active');

      btnLinguagens.style.border = '1px solid var(--color-border)';
      btnLinguagens.style.background = 'none';
      btnLinguagens.style.color = 'var(--color-text)';
      btnLinguagens.classList.remove('active');
    }
  };

  btnLinguagens?.addEventListener('click', () => {
    setAreaActiveTab('Linguagens');
    executarSorteioApendiceA();
  });

  btnHumanas?.addEventListener('click', () => {
    setAreaActiveTab('Ciências Humanas');
    executarSorteioApendiceA();
  });

  async function executarSorteioApendiceA() {
    tableArea.innerHTML = `
    ...
    try {
      ...
      let areaFilter = selectedAreaValue;
      if (areaFilter === 'Ciências Humanas') {
        areaFilter = 'Humanas';
      }
      ...
      if (questoesArea.length === 0) {
        throw new Error(`Nenhuma questão encontrada para a área: ${selectedAreaValue}`);
      }

      questoesArea.sort((a, b) => a.id.localeCompare(b.id));

      let seed = 2026;
      if (selectedAreaValue === 'Ciências Humanas') {
        seed = 2029;
      }
      ...
      if (selectedAreaValue !== 'Linguagens') {
      ...
      tableArea.innerHTML = `
        <div style="display:flex; justify-content: space-between; align-items:center; background: rgba(255,255,255,0.02); border: 1px solid var(--color-border); padding: 12px 18px; border-radius: 8px; margin-bottom: 16px;">
          <div style="font-size: 0.9rem; font-weight: 500; color: var(--color-text-secondary);">
            Distribuição da área <strong>${selectedAreaValue}</strong>: 
  ...
  // Carrega automaticamente a área padrão (Linguagens) ao inicializar a tela
  executarSorteioApendiceA();
  ...
  function getAreaCode(selectedQ) {
    ...
    if (
      materias.some((m) =>
        ['filosofia', 'sociologia', 'história', 'geografia', 'historia'].includes(m),
      )
    )
      return 'ch';
    if (
      ...
      return 'lc';
    if (materias.some((m) => ['física', 'química', 'biologia', 'fisica', 'quimica'].includes(m)))
      return 'cn';
  ...
  function matchQuestao(experimentoQ, selectedQ) {
    ...
    const isEnemSel =
      (creditos.material && creditos.material.toLowerCase().includes('enem')) ||
      selectedQ.prova.toLowerCase().includes('enem');

    const isFuvestExp = experimentoQ.id.toLowerCase().includes('fuvest');
    const isFuvestSel =
      (creditos.material && creditos.material.toLowerCase().includes('fuvest')) ||
    ...
    const selYear =
      ...
      (selectedQ.prova.match(/\d{4}/) ? selectedQ.prova.match(/\d{4}/)[0] : '');
    ...
    if (isEnemSel) {
      ...
      const expArea = experimentoQ.id.toLowerCase().includes('lc')
        ? 'lc'
        : experimentoQ.id.toLowerCase().includes('ch')
          ? 'ch'
          : experimentoQ.id.toLowerCase().includes('cn')
            ? 'cn'
            : experimentoQ.id.toLowerCase().includes('nat')
              ? 'cn'
              : experimentoQ.id.toLowerCase().includes('mt')
                ? 'mt'
                : experimentoQ.id.toLowerCase().includes('mat')
                  ? 'mt'
                  : '';
  ...
  async function atualizarMetadataArea() {
    ...
    if (state.selectedQuestion) {
      ...
      if (modeloDesejado) {
        const cleanLabel = modelLabel
          .replace(/\s*\((?:Groq|Vertex|Vertex MaaS)\)\s*/gi, '')
          .toLowerCase()
          .replace(/[^a-z0-9]/g, '');
  ...
  async function handleJsonFile(file) {
    ...
    try {
      ...
      if (!canonicalModel) {
        throw new Error(
          `Modelo "${parsed.model}" não é suportado pelo teste Crossover (Gemma 4 31B IT, GPT-OSS-120B, Gemini 3.5 Flash).`,
        );
      }
  ...
  async function runCrossEvaluation() {
    ...
    const judgeLabel =
      judgeId === 'vertex/gemini-3.5-flash'
        ? 'Gemini 3.5 Flash (Vertex)'
        : judgeId === 'models/gemini-3.5-flash'
          ? 'Gemini 3.5 Flash (AI Studio)'
          : judgeId.includes('gemma')
            ? 'Gemma 4 31B IT'
            : judgeId === 'vertex-maas/gpt-oss-120b'
              ? 'GPT-OSS-120B (Vertex MaaS)'
              : judgeId === 'groq/gpt-oss-120b'
                ? 'GPT-OSS-120B (Groq)'
                : 'GPT-OSS-120B';
    ...
    try {
      ...
      const correctOption = gObj.alternativa_correta
        ? `Alternativa Correta: ${gObj.alternativa_correta}`
        : '';
      ...
      const isMultiPartJudge =
        judgeId.toLowerCase().includes('gemma') ||
      ...
      if (isMultiPartJudge) {
        ...
        const onPartAction = (
          ...
          return new Promise((resolve) => {
            ...
            if (judgeId.includes('gpt-oss-120b')) {
              defaultNextModel =
                partNumber + 1 <= 3 ? 'vertex-maas/gpt-oss-120b' : 'groq/gpt-oss-120b';
            }
            ...
            if (judgeId.includes('gpt-oss-120b')) {
              ...
              if (selectRetry && labelRetry) {
                const updateRetryVis = () => {
                  labelRetry.style.display =
                    selectRetry.value === 'vertex-maas/gpt-oss-120b' ? 'flex' : 'none';
                };
              ...
              if (!isLast) {
                ...
                if (selectNext && labelNext) {
                  const updateNextVis = () => {
                    labelNext.style.display =
                      selectNext.value === 'vertex-maas/gpt-oss-120b' ? 'flex' : 'none';
                  };
      ...
      const modelKey = judgeId.includes('gemini')
        ? 'gemini_3_5_flash'
        : judgeId.includes('gemma')
          ? 'gemma_4_31b_it'
          : 'o1';
      ...
      if (
        ...
        abortSignal?.aborted ||
        evaluationAbortController?.signal?.aborted
      ) {
...
let dashboardJuizAtual = 'all'; // "all", "gpt-oss-120b", "gemini-3.5-flash", "gemma-4-31b-it", "gpt+gemini", "gpt+gemma", "gemma+gemini"
...
async function carregarDashboardApendiceA(area = 'total') {
  ...
  try {
    ...
    const fullStats = await response.json();

    // Filtra pelos juízes selecionados se houver pacote compilado
    const stats =
      fullStats.by_judge_filter && fullStats.by_judge_filter[dashboardJuizAtual]
        ? fullStats.by_judge_filter[dashboardJuizAtual]
        : fullStats;

    stats.errors = fullStats.errors;
    stats.area_label = fullStats.area_label;
...
function renderDashboardUIApendiceA(
  container,
  stats,
  areaKey = 'total',
  areaLabel = 'Total (Linguagens + Humanas)',
  areaEmoji = '📊',
) {
  ...
  const getJudgeCardsInfo = (filterKey, statsObj) => {
    const agree = statsObj.inter_rater_agreement;
    const agreePct =
      agree?.percent_agreement != null ? (agree.percent_agreement * 100).toFixed(1) + '%' : 'N/A';
    const kappaVal = agree?.fleiss_kappa != null ? agree.fleiss_kappa.toFixed(3) : 'N/A';

    if (filterKey === 'gpt-oss-120b') {
      return {
        h1: 'Juiz GPT-OSS-120B',
        l1: 'Status do Filtro',
        v1: 'Avaliador Isolado',
        c1: '#f97316',
        h2: "Fleiss' Kappa / Inter-rater",
        l2: 'Métrica de Acordo',
        v2: 'N/A (Juiz Único)',
        c2: 'var(--color-text-secondary)',
        h3: 'Perfil de Rigor',
        l3: 'Consistência Crítica',
        v3: 'Rigor Elevado (65-80)',
        c3: '#f97316',
      };
    }
    if (filterKey === 'gemini-3.5-flash') {
      return {
        h1: 'Juiz Gemini 3.5 Flash',
        l1: 'Status do Filtro',
        v1: 'Avaliador Isolado',
        c1: '#4e82ee',
        h2: "Fleiss' Kappa / Inter-rater",
        ...
        c2: 'var(--color-text-secondary)',
        h3: 'Perfil de Rigor',
        l3: 'Leniência Relativa',
        v3: 'Leniência (85-95)',
        c3: '#4e82ee',
      };
    }
    if (filterKey === 'gemma-4-31b-it') {
      return {
        h1: 'Juiz Gemma 4 31B IT',
        l1: 'Status do Filtro',
        v1: 'Avaliador Isolado',
        c1: '#a75df4',
        h2: "Fleiss' Kappa / Inter-rater",
    ...
    if (filterKey === 'gpt+gemini') {
      return {
        h1: 'Par: GPT-OSS + Gemini 3.5',
        l1: 'Concordância Inter-rater',
        v1: agreePct,
        c1: '#4e82ee',
        h2: 'Par: GPT-OSS + Gemini 3.5',
        l2: "Fleiss' Kappa (Reliability)",
        v2: kappaVal,
        c2: '#a75df4',
        h3: 'Consistência da Auditoria',
        l3: 'Combinação Híbrida',
        v3: 'Moderada/Limiar',
        c3: '#f97316',
      };
    }
    if (filterKey === 'gpt+gemma') {
      return {
        h1: 'Par: GPT-OSS + Gemma 4',
        l1: 'Concordância Inter-rater',
        v1: agreePct,
        c1: '#4e82ee',
        h2: 'Par: GPT-OSS + Gemma 4',
        l2: "Fleiss' Kappa (Reliability)",
        v2: kappaVal,
        c2: '#a75df4',
        h3: 'Consistência da Auditoria',
    ...
    if (filterKey === 'gemma+gemini') {
      return {
        h1: 'Par: Gemma 4 + Gemini 3.5',
        l1: 'Concordância Inter-rater',
        v1: agreePct,
        c1: '#4e82ee',
        h2: 'Par: Gemma 4 + Gemini 3.5',
        l2: "Fleiss' Kappa (Reliability)",
        v2: kappaVal,
        c2: '#a75df4',
        h3: 'Consistência da Auditoria',
        l3: 'Combinação Leniênte',
        v3: 'Baixa/Divergente',
        c3: '#dc3545',
      };
    }
    return {
      h1: 'Consolidado Geral',
      l1: 'Concordância Inter-rater Global',
      v1: agreePct,
      c1: '#4e82ee',
      h2: 'Consolidado Geral',
      l2: "Fleiss' Kappa Global",
      v2: kappaVal,
      c2: '#a75df4',
      h3: 'Auditoria Crossover',
      l3: 'Todos os 3 Juízes',
      v3: 'Moderada/Limiar',
      c3: '#f97316',
    };
  };

  const kpiInfo = getJudgeCardsInfo(dashboardJuizAtual, stats);
  ...
  container.innerHTML = `
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; align-items: center;">
      <span style="font-size:0.8rem; font-weight:bold; color:var(--color-text-secondary); margin-right:4px;">Filtrar por área:</span>
      ...
      <span style="font-size:0.8rem; font-weight:bold; color:var(--color-text-secondary); margin-left: 12px; margin-right: 4px;">Juiz Avaliador:</span>
      <select id="selectFiltroJuiz" style="padding: 6px 12px; border-radius: 20px; border: 1px solid var(--color-border); background: var(--color-surface); color: var(--color-text); font-weight: bold; font-size: 0.85rem; cursor: pointer;">
        <option value="all" ${dashboardJuizAtual === 'all' ? 'selected' : ''}>🌟 Todos os Juízes (Consolidado)</option>
        <option value="gpt-oss-120b" ${dashboardJuizAtual === 'gpt-oss-120b' ? 'selected' : ''}>🟧 Apenas GPT-OSS-120B</option>
        <option value="gemini-3.5-flash" ${dashboardJuizAtual === 'gemini-3.5-flash' ? 'selected' : ''}>🟦 Apenas Gemini 3.5 Flash</option>
        <option value="gemma-4-31b-it" ${dashboardJuizAtual === 'gemma-4-31b-it' ? 'selected' : ''}>🟪 Apenas Gemma 4 31B IT</option>
        <option value="gpt+gemini" ${dashboardJuizAtual === 'gpt+gemini' ? 'selected' : ''}>🟧🟦 GPT-OSS + Gemini 3.5</option>
        <option value="gpt+gemma" ${dashboardJuizAtual === 'gpt+gemma' ? 'selected' : ''}>🟧🟪 GPT-OSS + Gemma 4</option>
        <option value="gemma+gemini" ${dashboardJuizAtual === 'gemma+gemini' ? 'selected' : ''}>🟪🟦 Gemma 4 + Gemini 3.5</option>
      </select>

      <span id="macro-tab-subtitle" style="font-size:0.78rem; color:var(--color-text-secondary); margin-left:8px; font-style:italic;">${areaEmoji} ${areaLabel} &mdash; N=${stats.n_total} questões pareadas</span>
    </div>
      ...
      <div class="ap-tab-content" id="ap-tab-gerais" style="display: block;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px; margin-bottom: 20px;">
          ...
          <div class="stats-card-kpi">
            ...
            <div class="kpi-val" style="color: ${stats.kruskal_wallis?.pValue != null && stats.kruskal_wallis.pValue < 0.05 ? 'var(--color-success)' : 'var(--color-text)'};">${stats.kruskal_wallis?.pValue != null ? stats.kruskal_wallis.pValue.toFixed(4) : 'N/A'}</div>
            <div style="font-size:0.7rem; color:var(--color-text-secondary);">${stats.kruskal_wallis?.pValue != null && stats.kruskal_wallis.pValue < 0.05 ? 'Variabilidade estatisticamente real' : 'Sem variabilidade real'}</div>
          </div>
        </div>
        ...
        <div class="chart-card-full">
          <h4>5, 6 e 7. Testes Estatísticos Avançados (Wilcoxon, Hedges' g, Permutação Exata & Win Rate)</h4>
          <table class="stats-table-compact">
            <thead>
              <tr>
                ...
                <th>Delta (Δ)</th>
                <th>Hedges' g (Efeito)</th>
                <th>Win Rate (%)</th>
                <th>Permutação Exata (p)</th>
                <th>Wilcoxon p</th>
              </tr>
            </thead>
            <tbody>
              ${Object.keys(stats.metrics)
                .filter((k) => k !== 'iep')
                .map((k) => {
                  ...
                  const hg =
                    m.hedges_g != null
                      ? m.hedges_g >= 0
                        ? '+' + m.hedges_g.toFixed(4)
                        : m.hedges_g.toFixed(4)
                      : 'N/A';
                  const wr = m.win_rate
                    ? `${m.win_rate.winRatePct.toFixed(1)}% (${m.win_rate.wins}V/${m.win_rate.losses}D)`
                    : 'N/A';
                  const permP =
                    m.exact_permutation?.pValue != null
                      ? m.exact_permutation.pValue.toFixed(4)
                      : 'N/A';
                  const wilcP = m.wilcoxon.pValue !== null ? m.wilcoxon.pValue.toFixed(4) : 'N/A';
                  return `
                  <tr>
                    ...
                    <td style="font-weight:bold; color: ${m.hedges_g > 0.5 ? 'var(--color-success)' : m.hedges_g < -0.5 ? 'var(--color-error)' : 'inherit'};">${hg}</td>
                    <td>${wr}</td>
                    <td>${permP}</td>
                    <td style="color:${m.wilcoxon.significant ? 'var(--color-success)' : 'inherit'}; font-weight:${m.wilcoxon.significant ? 'bold' : 'normal'};">${wilcP}</td>
                  </tr>
                `;
                })
                .join('')}
        ...
        <div class="chart-card-full" style="margin-top:20px;gap: 20px;display: flex;flex-direction: column;">
          <h4>8. Análise de Cutoff Avançada (Pré-Cutoff <=2024 vs. Pós-Cutoff 2025 Inéditas)</h4>
          <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap:20px;">
            <div>
              <h5 style="margin:0 0 10px 0; color:var(--color-primary);">Visão Geral do Cutoff (Nota Total)</h5>
              <table class="stats-table-compact">
                <thead>
                  <tr>
                    <th>Período</th>
                ...
                <tbody>
                  ${['pre_cutoff', 'post_cutoff']
                    .map((cKey) => {
                      const cData = stats.cutoff_analysis[cKey];
                      if (!cData) return '';
                      const label =
                        cKey === 'pre_cutoff'
                          ? 'Pré-Cutoff (<=2024)'
                          : 'Pós-Cutoff (2025 Inéditas)';
                      const hg =
                        cData.hedges_g != null
                          ? cData.hedges_g >= 0
                            ? '+' + cData.hedges_g.toFixed(4)
                            : cData.hedges_g.toFixed(4)
                          : 'N/A';
                      const wr = cData.win_rate
                        ? `${cData.win_rate.winRatePct.toFixed(1)}% (${cData.win_rate.wins}V/${cData.win_rate.losses}D)`
                        : 'N/A';
                      const wilcP =
                        cData.wilcoxon?.pValue != null ? cData.wilcoxon.pValue.toFixed(4) : 'N/A';
                      return `
                      <tr>
                        <td><strong>${label}</strong></td>
                        <td>${cData.n}</td>
                        <td>${cData.control.mean.toFixed(2)}</td>
                        <td>${cData.experimental.mean.toFixed(2)}</td>
                        <td>${hg}</td>
                        <td>${wr}</td>
                        <td>${wilcP}</td>
                      </tr>
            ...
            <div>
              <h5 style="margin:0 0 10px 0; color:#a75df4;">Desagregação por Grupo no Pós-Cutoff (2025, N=${stats.cutoff_analysis?.post_cutoff?.n || 7})</h5>
              <table class="stats-table-compact">
                <thead>
                  <tr>
                    <th>Grupo</th>
                ...
                <tbody>
                  ${Object.entries(stats.cutoff_analysis?.post_cutoff?.groups_breakdown || {})
                    .map(([gKey, gData]) => {
                      const gLabels = {
                        grupo_a: 'Grupo A (Estética)',
                        grupo_b: 'Grupo B (Ancoragem)',
                        grupo_c: 'Grupo C (Didática)',
                        grupo_d: 'Grupo D (Lógica)',
                        grupo_e: 'Grupo E (Interface/UX)',
                      };
                      const hg =
                        gData.hedges_g >= 0
                          ? '+' + gData.hedges_g.toFixed(4)
                          : gData.hedges_g.toFixed(4);
                      const wr = `${gData.win_rate.winRatePct.toFixed(1)}% (${gData.win_rate.wins}V/${gData.win_rate.losses}D)`;
                      const highlight = gKey === 'grupo_c' || gKey === 'grupo_e';
                      return `
                      <tr style="${highlight ? 'background: rgba(167,93,244,0.08); font-weight:bold;' : ''}">
                        <td><strong>${gLabels[gKey] || gKey}</strong></td>
                        <td>${gData.control_mean.toFixed(2)}</td>
                        <td>${gData.experimental_mean.toFixed(2)}</td>
                        <td style="color:${gData.hedges_g > 0.5 ? 'var(--color-success)' : gData.hedges_g < -0.5 ? 'var(--color-error)' : 'inherit'};">${hg}</td>
                        <td style="color:${gData.win_rate.winRatePct > 50 ? 'var(--color-success)' : 'inherit'};">${wr}</td>
                      </tr>
                    `;
                    })
                    .join('')}
      ...
      <div class="ap-tab-content" id="ap-tab-modelos" style="display: none;">
        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:15px; margin-bottom:20px;">
          <div class="stats-card-kpi" style="background: rgba(78, 130, 238, 0.04);">
            <div style="font-size:0.75rem; text-transform:uppercase; color:${kpiInfo.c1}; font-weight:bold;">${kpiInfo.h1}</div>
            <div style="font-size: 0.85rem; margin-top:5px; color:var(--color-text);">${kpiInfo.l1}</div>
            <div class="kpi-val" style="color:${kpiInfo.c1};">${kpiInfo.v1}</div>
          </div>
          <div class="stats-card-kpi" style="background: rgba(167, 93, 244, 0.04);">
            <div style="font-size:0.75rem; text-transform:uppercase; color:${kpiInfo.c2}; font-weight:bold;">${kpiInfo.h2}</div>
            <div style="font-size: 0.85rem; margin-top:5px; color:var(--color-text);">${kpiInfo.l2}</div>
            <div class="kpi-val" style="color:${kpiInfo.c2};">${kpiInfo.v2}</div>
          </div>
          <div class="stats-card-kpi" style="background: rgba(249, 115, 22, 0.04);">
            <div style="font-size:0.75rem; text-transform:uppercase; color:${kpiInfo.c3}; font-weight:bold;">${kpiInfo.h3}</div>
            <div style="font-size: 0.85rem; margin-top:5px; color:var(--color-text);">${kpiInfo.l3}</div>
            <div class="kpi-val" style="color:${kpiInfo.c3};">${kpiInfo.v3}</div>
          </div>
        </div>
  ...
  // Event Listener para a troca do filtro de juízes
  const selectJuiz = container.querySelector('#selectFiltroJuiz');
  if (selectJuiz) {
    selectJuiz.addEventListener('change', (e) => {
      dashboardJuizAtual = e.target.value;
      dashboardAreaAtual = null; // Força re-renderização da área com novo filtro
      carregarDashboardApendiceA(areaKey);
    });
  }
...
function renderTabCharts(tabName, container, stats) {
  ...
  const getRateCtrl = (key) =>
    (qData.reduce((sum, q) => sum + (q.criterios_control[key]?.presence_rate || 0), 0) /
      stats.n_total) *
    100;
  const getRateExp = (key) =>
    (qData.reduce((sum, q) => sum + (q.criterios_experimental[key]?.presence_rate || 0), 0) /
      stats.n_total) *
    100;

  const createSafeChart = (canvasId, config) => {
    const el = typeof canvasId === 'string' ? document.getElementById(canvasId) : canvasId;
    if (!el) return null;
    const existing = window.Chart && window.Chart.getChart(el);
    if (existing) {
      existing.destroy();
    }
  ...
  if (tabName === 'gerais') {
    ...
    createSafeChart('chart-radar-grupos', {
      type: 'radar',
      data: {
    ...
    createSafeChart('chart-linha-acumulada', {
      type: 'line',
      data: {
    ...
    const cLabels = cKeys.map((k) =>
      k.replace(/_/g, ' ').replace(/\b(a|b|c|d|e)\b/g, (m) => m.toUpperCase()),
    );
    ...
    createSafeChart('chart-checklist-conformity', {
      type: 'bar',
      data: {
        labels: cLabels,
        datasets: [
    ...
    createSafeChart('chart-pedagogia-avancada', {
      type: 'bar',
      data: {
        ...
        datasets: [
          {
            label: 'Controle',
            data: [
              getRateCtrl('recurso_didatico_avancado_analogia'),
              getRateCtrl('metodologia_de_resolucao_explicita'),
            ],
            backgroundColor: 'rgba(98, 104, 113, 0.7)',
          },
          {
            label: 'Maia.edu',
            data: [
              getRateExp('recurso_didatico_avancado_analogia'),
              getRateExp('metodologia_de_resolucao_explicita'),
            ],
            backgroundColor: 'rgba(33, 128, 141, 0.7)',
          },
        ],
    ...
    const errRates = ancKeys.map(
      (k) =>
        (1 -
          qData.reduce((sum, q) => sum + (q.criterios_experimental[k]?.presence_rate || 0), 0) /
            stats.n_total) *
        100,
    );

    createSafeChart('chart-ancoragem-erros', {
      type: 'bar',
      data: {
    ...
    createSafeChart('chart-con-redundancia', {
      type: 'doughnut',
      data: {
    ...
    drawSimpleBar(
      'chart-des-pegadinhas',
      'Controle',
      getRateCtrl('mecanismo_do_erro_nos_distratores'),
      'Maia.edu',
      getRateExp('mecanismo_do_erro_nos_distratores'),
    );

    // 15. Formatação
    const formatRate = getRateExp('ausencia_de_tags_corrompidas');
    createSafeChart('chart-tags-quebradas', {
      type: 'doughnut',
      data: {
    ...
    createSafeChart('chart-distratores-refutacao', {
      type: 'bar',
      data: {
        labels: ['A', 'B', 'C', 'Outros'],
        datasets: [
          {
            label: 'Controle',
            data: [
              getRateCtrl('analise_isolada_distrator_A'),
              getRateCtrl('analise_isolada_distrator_B'),
              getRateCtrl('analise_isolada_distrator_C'),
              getRateCtrl('analise_isolada_distrator_restante'),
            ],
            backgroundColor: 'rgba(98, 104, 113, 0.7)',
          },
          {
            label: 'Maia.edu',
            data: [
              getRateExp('analise_isolada_distrator_A'),
              getRateExp('analise_isolada_distrator_B'),
              getRateExp('analise_isolada_distrator_C'),
              getRateExp('analise_isolada_distrator_restante'),
            ],
            backgroundColor: 'rgba(33, 128, 141, 0.7)',
          },
        ],
    ...
    drawSimpleBar(
      'chart-arcabouco-nominal',
      'Controle',
      getRateCtrl('aplicacao_nominal_arcabouco_teorico'),
      'Maia.edu',
      getRateExp('aplicacao_nominal_arcabouco_teorico'),
    );

    // 18. Clareza Gabarito
    const gabRate = getRateExp('declaracao_gabarito_indica_letra');
    createSafeChart('chart-clareza-gabarito', {
      type: 'doughnut',
      data: {
    ...
    // 20. Recursos Visuais
    drawSimpleBar(
      'chart-recursos-didaticos-visuais',
      'Controle',
      getRateCtrl('recurso_visual_de_destaque'),
      'Maia.edu',
      getRateExp('recurso_visual_de_destaque'),
    );

    // 21. Definição Termos
    drawSimpleBar(
      'chart-definicao-termos',
      'Controle',
      getRateCtrl('definicao_de_termos_chave'),
      'Maia.edu',
      getRateExp('definicao_de_termos_chave'),
    );

    // 22. Cadeia Explicativa
    drawSimpleBar(
      'chart-ausencia-saltos',
      'Controle',
      getRateCtrl('ausencia_de_saltos_logicos'),
      'Maia.edu',
      getRateExp('ausencia_de_saltos_logicos'),
    );
    ...
    qData.forEach((q) => {
      (q.raw_evaluations?.experimental || []).forEach((e) => {
        const jName = e.judge || e.model;
        const val = e.pontuacao_total !== undefined ? e.pontuacao_total : e.total;
        if (typeof val === 'number' && !isNaN(val)) {
          if (jName === 'gemini-3.5-flash') geminiDada.push(val);
          if (jName === 'gemma-4-31b-it') gemmaDada.push(val);
          if (jName === 'gpt-oss-120b') gptDada.push(val);
        }
      });
    });

    const boxTraces = [];
    if (geminiDada.length > 0)
      boxTraces.push({
        y: geminiDada,
        type: 'box',
    ...
    Plotly.newPlot(
      'chart-scatter-latencia-modelos',
      scatterTraces,
      {
        ...plotlyLayoutBase,
    ...
    Plotly.newPlot(
      'chart-word-density-evidencia',
      [
        {
          y: trueLengths,
          type: 'violin',
          name: 'Marcados como Presentes',
          marker: { color: '#21808d' },
        },
        {
          y: falseLengths,
          type: 'violin',
          name: 'Marcados como Ausentes',
          marker: { color: '#dc3545' },
        },
      ],
      { ...plotlyLayoutBase },
    ...
    createSafeChart('chart-pie-latencias', {
      type: 'pie',
      data: {
    ...
    createSafeChart('chart-latencia-sessao', {
      type: 'line',
      data: {
        labels: qData.map((_, idx) => `Q${idx + 1}`),
        datasets: [
    ...
    const speedsExp = qData.map(
      (q) => q.char_count_experimental / (q.latencies_experimental.generation_ms || 1),
    );

    createSafeChart('chart-velocidade-escrita', {
      type: 'bar',
      data: {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: textColor } },
          y: {
            title: { display: true, text: 'Caracteres por segundo', color: textColor },
    ...
    const indepSuccess = qData.map(
      (q) => q.criterios_experimental.independencia_da_memoria_parametrica?.presence_rate || 0,
    );

    Plotly.newPlot(
      'chart-memory-vs-independence',
      [
        {
          x: memoryTimes,
          y: indepSuccess,
    ...
    const latByHour = hours.map((h) => {
      ...
      return items.length > 0
        ? getMean(items.map((q) => q.latencies_experimental.total_ms / 1000))
        : null;
    });

    createSafeChart('chart-latencia-horario', {
      type: 'line',
      data: {
    ...
    Plotly.newPlot(
      ...
      [
        ...
        },
        {
          ...
          y: trendY,
          mode: 'lines',
          name: `Tendência (R²=${reg.r2.toFixed(3)})`,
    ...
    const iepValues = qData.map((q) => {
      ...
      return q.latencies_experimental.total_ms > 0
        ? delta / (q.latencies_experimental.total_ms / 1000)
        : 0;
    });

    Plotly.newPlot(
      'chart-iep-pure-scatter',
      [
        {
