...
export const PdfEmbedRenderer: React.FC<PdfEmbedRendererProps> = (props) => {
  ...
  const origWidth = isPercentWidth
    ...
    : typeof c.width === 'number'
      ? c.width
      : parseInt(c.width as string) || 600;
  ...
  useEffect(() => {
    ...
    const calculateScale = () => {
      ...
      const heightScale =
        heightAfterWidthScale > maxAllowedHeight ? maxAllowedHeight / heightAfterWidthScale : 1;
      ...
      if (event.data?.type === 'pdfViewerReady') {
        ...
        if (event.data.success) {
          ...
          if (event.data.contentHeight) {
            console.log(
              '[PdfEmbed] Recebeu dimensões do viewer:',
          ...
          if (event.data.imageDataUrl) {
            ...
            window.dispatchEvent(
              new CustomEvent('pdf-embed-crop-ready', {
                detail: {
                  url: effectiveUrl,
                  page: pageNum,
                  x: c.X,
  ...
  useEffect(() => {
    ...
    const handleCropReady = (e: Event) => {
      ...
      if (
        ...
        console.log(
          '[PdfEmbed] Componente atualizou cachedImage via evento:',
          detail.dataUrl.substring(0, 50) + '...',
        );
    ...
    try {
      ...
      if (!canvas) {
        ...
        if (retryRender >= MAX_RETRIES) {
          console.error(
            '[PdfEmbed] Canvas não está disponível após ' + MAX_RETRIES + ' tentativas',
          );
        ...
        console.warn(
          '[PdfEmbed] Canvas não está montado, agendando re-tentativa... (' +
            (retryRender + 1) +
            '/' +
            MAX_RETRIES +
            ')',
    ...
    const urlInputBar = (
      ...
      >
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') handleManualUrlSubmit();
          }}
          style={{
            flex: 1,
            padding: '8px 12px',
            fontSize: '11px',
            background: 'var(--color-bg-1, #1e293b)',
      ...
      return (
        <div ref={containerRef} style={{ position: 'relative', width: '100%', minHeight: '120px' }}>
          ...
          >
            ...
            >
              Nenhum link de PDF encontrado — cole abaixo para visualizar
            </span>
            {urlInputBar}
          </div>
        </div>
  ...
  const hasLocalFile =
    (typeof window !== 'undefined' && !!(window as any).__pdfLocalFile) || hasUploadedFile;
  ...
  const isRecoverableError =
    fallbackError === 'NEEDS_UPLOAD' || fallbackError?.includes('CORS_BLOCK');
  const shouldShowUpload =
    !isLoadingLocal &&
    ((isRecoverableError && !hasLocalFile) || (!effectiveUrl && !hasLocalFile && !isLoadingLocal));

  if (shouldShowUpload) {
    // Logic for determining the message type
    const needsUpload =
      fallbackError === 'NEEDS_UPLOAD' || fallbackError?.includes('CORS_BLOCK') || !effectiveUrl;
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              >
                {props.readOnly
                  ...
                  : !effectiveUrl
                    ? 'Arquivo sem link - selecione o PDF original'
