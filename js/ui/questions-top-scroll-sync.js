  ...
  function setupDrag(thumb, track, isVertical) {
    ...
    function handleStart(e) {
      ...
      const moveHandler = (ev) => {
        ...
        const thumbDim = isVertical
          ? parseFloat(thumb.style.height)
          : parseFloat(thumb.style.width);
        ...
        if (trackScrollable > 0) {
          if (isVertical)
            container.scrollTop = startScroll + (delta / trackScrollable) * scrollable;
...
export function destroyQuestionsTopScrollSync() {
  document
    .querySelectorAll('.questions-top-scroll-track, .questions-vertical-scroll-track')
