...
export function resolveCssVarsInChart(chartCode) {
  ...
  return chartCode.replace(
    ...
    (match, varName, fallback) => {
      const value = rootStyle.getPropertyValue(varName.trim()).trim();
...
const observer = new MutationObserver((mutations) => {
  for (const mutation of mutations) {
    if (
      ...
      (mutation.attributeName === 'style' || mutation.attributeName === 'data-color-scheme')
    ) {
      debouncedRerender();
      break;
    }
...
export async function hydrateAllChatContent(container) {
  ...
  const hydrationPromises = Array.from(placeholders).map(async (placeholder) => {
    try {
      // Evita re-hidratação
    ...
    const renderImage = (imgUrl) => {
      ...
      if (imgEl) {
        imgEl.addEventListener(
          'error',
          () => {
            console.warn(`[Hydration] ⚠️ Imagem bloqueada/indisponível no DOM: ${imgUrl}`);
            ...
            if (fallbackRetries < MAX_FALLBACK_RETRIES) {
              ...
              renderLoadingState(
                `Buscando alternativa (${fallbackRetries}/${MAX_FALLBACK_RETRIES})...`,
              );
        ...
        imgEl.addEventListener('click', () => {
          ...
          closeBtn.style.border = '1px solid var(--color-border, rgba(255,255,255,0.1))';
        ...
        if (chat && chat.messages && chat.messages[msgIndex]) {
          ...
          const updateBlock = (block) => {
            ...
            if (block.tipo === 'imagem') {
              const blockConteudo =
                typeof block.conteudo === 'string' ? block.conteudo : block.query || '';
              if (
            ...
            if (Array.isArray(block)) {
              ...
              const keysToScan = ['conteudo', 'slots', 'sections', 'slides', 'content'];
              keysToScan.forEach((key) => {
                if (block[key]) {
                  if (Array.isArray(block[key])) block[key].forEach(updateBlock);
          ...
          if (modified) {
            ...
            console.log('[Hydration] ✅ URL da Imagem salva no histórico:', imgUrl);
          }
  ...
  const mermaidPromises = Array.from(mermaidPlaceholders).map(async (placeholder, idx) => {
    placeholder.dataset.hydrated = 'true';
    ...
    try {
      ...
      if (processedChartCode.toLowerCase().trim().startsWith('mindmap')) {
        ...
        const themePrimary =
          ensureHexColor(rootStyle.getPropertyValue('--color-primary').trim()) || '#cc5252';
        const themeSecondary =
          ensureHexColor(rootStyle.getPropertyValue('--color-secondary').trim()) || '#b84dff';
        const themeAccent =
          ensureHexColor(rootStyle.getPropertyValue('--color-accent').trim()) || '#4d94ff';
...
export async function hydrateSources(container) {
  ...
  if (sourceCards.length > 0) {
      ...
      await Promise.all(
        batch.map(async (card) => {
          ...
          try {
            ...
            const finalUrl =
              typeof resolved === 'object' && resolved?.resolved
                ? resolved.resolved
            ...
            try {
              ...
              /* microlink is optional, never blocks */
            }
    ...
    try {
      ...
      const compactFooter = sourcesContainer
        .closest('.chat-sources-container')
