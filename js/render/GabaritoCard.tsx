...
// @ts-expect-error - Importação legado pode não ter tipos
...
export const MetaGabarito: React.FC<{
  ...
  const hasConfianca =
    confianca !== null && confianca !== undefined && !Number.isNaN(Number(confianca));
  ...
  const origemStateClass =
    origemState === 'approved'
      ? 'block-approved'
      : origemState === 'rejected'
        ? 'block-rejected'
        : '';
  ...
  return (
    <div className="gabarito-meta">
      <div className="gabarito-meta__row" style={{ gap: '10px' }}>
        {hasConfianca && (
          <div className="gabarito-chip gabarito-chip--info">
            ...
            <span className="gabarito-chip__v">
              <SafeText text={fmtPct(confianca)} />
            </span>
          </div>
        )}
        {hasOrigem &&
          (isReviewMode && onApprove && onReject ? (
            <div
              className={`gabarito-chip gabarito-chip--muted reviewable-chip ${origemStateClass}`}
              style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              <span className="gabarito-chip__k">Origem</span>
              <span className="gabarito-chip__v">
                <SafeText text={creditos?.origemresolucao} />
              </span>
              <div className="review-btn-group" style={{ marginLeft: '4px' }}>
                <button
                  type="button"
                  className={`review-btn review-btn--approve review-btn--xs ${origemState === 'approved' ? 'active' : ''}`}
                  onClick={() => onApprove(origemFieldId)}
                >
                  ✓
                </button>
                <button
                  type="button"
                  className={`review-btn review-btn--reject review-btn--xs ${origemState === 'rejected' ? 'active' : ''}`}
                  onClick={() => onReject(origemFieldId)}
                >
                  ✗
                </button>
              </div>
      ...
      {hasConfianca && (
        <div
          className="gabarito-confbar"
          style={{ '--fill-width': safe(fill) } as React.CSSProperties}
        >
      ...
      {alts.map((alt: any, idx: number) => {
        ...
        const altStateClass =
          altState === 'approved'
            ...
            : altState === 'rejected'
              ? 'block-rejected'
              : '';
        const motivoStateClass =
          motivoState === 'approved'
            ? 'block-approved'
            : motivoState === 'rejected'
              ? 'block-rejected'
              : '';
        ...
        if (isReviewMode && onApprove && onReject) {
          return (
            <div
              key={idx}
              className="answerOption-wrapper"
              style={{ display: 'flex', alignItems: 'stretch', gap: '8px' }}
            >
              {/* Alternativa com layout original */}
              <div
                className={`answerOption ${isCorrect ? 'correct' : ''} ${altStateClass}`}
                style={{ flex: 1 }}
              >
                ...
                <div className="option-text">
                  ...
                  {analise?.motivo && (
                    <div
                      className={`reviewable-block ${motivoStateClass}`}
                      style={{
                        marginTop: '10px',
                        padding: '10px',
                        borderRadius: '8px',
              ...
              <div
                className="review-btn-group-vertical"
                style={{
                  display: 'flex',
  ...
  return (
    <div className="passo gabarito-steps">
      <div className="passoText">
        <p>
          <strong>Explicação (passo a passo)</strong>
        </p>
      </div>
      <div className="explicacao">
        <ol className="steps-list">
          {explicacaoArray.map((p, idx) => {
            ...
            if (!(window as any).__imagensLimpas.gabarito_passos)
              (window as any).__imagensLimpas.gabarito_passos = {};
            ...
            const origemStateClass =
              origemState === 'approved'
                ...
                : origemState === 'rejected'
                  ? 'block-rejected'
                  : '';
            const evidenciaStateClass =
              evidenciaState === 'approved'
                ? 'block-approved'
                : evidenciaState === 'rejected'
                  ? 'block-rejected'
                  : '';
            ...
            if (isReviewMode && onApprove && onReject) {
              return (
                ...
                >
                  ...
                  <div
                    className={`step-card ${passoStateClass}`}
                  ...
                  >
                    ...
                    <div className="step-body">
                      ...
                      >
                        {/* Origem - revisável */}
                        <div
                          className={`step-chip-wrapper ${origemStateClass}`}
                          style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                            gap: '4px',
                        ...
                        {p?.fontematerial && (
                          <span
                            className="step-chip step-chip--muted"
                            title={`Fonte/Material: ${safe(p.fontematerial)}`}
                          >
                        ...
                        {p?.evidencia && (
                          <div
                            className={`step-chip-wrapper ${evidenciaStateClass}`}
                            style={{
                              display: 'inline-flex',
                              alignItems: 'center',
                              gap: '4px',
                          ...
                          >
                            <span
                              className="step-chip step-chip--muted"
                              title={`Evidência Visual: ${safe(p.evidencia)}`}
                            ...
                            <div className="review-btn-group">
                              <button
                                type="button"
                                className={`review-btn review-btn--approve review-btn--xs ${evidenciaState === 'approved' ? 'active' : ''}`}
                                onClick={() => onApprove(evidenciaFieldId)}
                              ...
                              <button
                                type="button"
                                className={`review-btn review-btn--reject review-btn--xs ${evidenciaState === 'rejected' ? 'active' : ''}`}
                                onClick={() => onReject(evidenciaFieldId)}
                  ...
                  >
                    <button
                      type="button"
                      className={`review-btn review-btn--approve review-btn--xs ${passoState === 'approved' ? 'active' : ''}`}
                      onClick={() => onApprove(passoFieldId)}
                      title={`Aprovar passo ${idx + 1}`}
                    >
                      ✓
                    </button>
                    <button
                      type="button"
                      className={`review-btn review-btn--reject review-btn--xs ${passoState === 'rejected' ? 'active' : ''}`}
                      onClick={() => onReject(passoFieldId)}
                      title={`Rejeitar passo ${idx + 1}`}
                    >
                      ✗
                    </button>
                  </div>
        ...
        {fontes.map((fonte, idx) => {
          ...
          if (isReviewMode && onApprove && onReject) {
            return (
              <li
                key={idx}
                className={`fonte-wrapper ${fonteStateClass}`}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '8px',
              ...
              >
                ...
                <div className="review-btn-group">
                  <button
                    type="button"
                    className={`review-btn review-btn--approve review-btn--xs ${fonteState === 'approved' ? 'active' : ''}`}
                    onClick={() => onApprove(fonteFieldId)}
                    title="Aprovar fonte"
                  >
                    ✓
                  </button>
                  <button
                    type="button"
                    className={`review-btn review-btn--reject review-btn--xs ${fonteState === 'rejected' ? 'active' : ''}`}
                    onClick={() => onReject(fonteFieldId)}
                    title="Rejeitar fonte"
                  >
                    ✗
                  </button>
                </div>
...
export const DetalhesTecnicos: React.FC<{
  ...
  const ReviewableChip = ({
    ...
    if (isReviewMode && onApprove && onReject) {
      return (
        <div
          className={`chip-wrapper ${stateClass}`}
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '4px',
  ...
  const Chip = ({ label, ok, okTxt = 'OK', badTxt = 'Atenção', fieldId }: any) => {
    const chip = (
      <div className={`coerencia-chip ${ok ? 'coerencia-chip--ok' : 'coerencia-chip--bad'}`}>
        <span className="coerencia-chip-k">
          <SafeText text={label} />
        </span>
        <span className="coerencia-chip-v">
          <SafeText text={ok ? okTxt : badTxt} />
        </span>
      </div>
    );
    ...
    if (fieldId) {
      return (
        <ReviewableChip fieldId={fieldId} label={label}>
          {chip}
        </ReviewableChip>
      );
    }
  ...
  const ChipKV = ({ k, v, cls = '', fieldId }: any) => {
    ...
    if (fieldId) {
      return (
        <ReviewableChip fieldId={fieldId} label={k}>
          {chip}
        </ReviewableChip>
      );
    }
  ...
  const isDissertativa =
    dados.questao?.tipo_resposta === 'dissertativa' ||
    !dados.questao?.alternativas ||
    dados.questao.alternativas.length === 0;

  // Filtra logs inúteis de questões dissertativas gerados pela lógica da IA ou backend
  const filteredCoerenciaObs = coerenciaObs.filter(
    (o: string) =>
      !o.toLowerCase().includes('questão dissertativa') &&
      !o.toLowerCase().includes('alternativa correta está vazia') &&
      !o.toLowerCase().includes('alternativa_correta está vazia'),
  );
      ...
      {coerencia && (
        <div className="field-group">
          ...
          <div className="coerencia-grid">
            {!isDissertativa && (
              <Chip
                fieldId="detalhes_coerencia_alt_correta"
                label="Alternativa correta existe"
                ok={!!(coerencia.alternativa_correta_existe ?? coerencia.alternativaCorretaExiste)}
              />
            )}
            {!isDissertativa && (
              <Chip
                fieldId="detalhes_coerencia_analise_todas"
                label="Análise para todas"
                ok={!!(coerencia.tem_analise_para_todas ?? coerencia.temAnaliseParaTodas)}
              />
            )}
            <Chip
              fieldId="detalhes_coerencia_observacoes"
              label="Observações"
              ok={filteredCoerenciaObs.length === 0}
              okTxt="Nenhuma"
              badTxt="Há itens"
            />
          </div>
                ...
                {filteredCoerenciaObs.map((o: string, i: number) => {
                  ...
                  if (isReviewMode && onApprove && onReject) {
                    return (
                      <li
                        key={i}
                        className={`obs-item ${obsStateClass}`}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: '8px',
                      ...
                      >
                        ...
                        <div className="review-btn-group">
                          <button
                            type="button"
                            className={`review-btn review-btn--approve review-btn--xs ${obsState === 'approved' ? 'active' : ''}`}
                            onClick={() => onApprove(obsFieldId)}
                          >
                            ✓
                          </button>
                          <button
                            type="button"
                            className={`review-btn review-btn--reject review-btn--xs ${obsState === 'rejected' ? 'active' : ''}`}
                            onClick={() => onReject(obsFieldId)}
                          >
                            ✗
                          </button>
                        </div>
      ...
      {creditos && (
        <div className="field-group">
          ...
          <div className="coerencia-grid">
            ...
            <ChipKV
              fieldId="detalhes_creditos_material_identificado"
              k="Material identificado"
              v={creditos.materialidentificado ? 'Sim' : 'Não'}
              cls={creditos.materialidentificado ? 'coerencia-chip--ok' : 'coerencia-chip--bad'}
            />
            {/* Confiança ident. SEM revisão */}
            {creditos.confiancaidentificacao != null && (
              <ChipKV k="Confiança ident." v={toPct(creditos.confiancaidentificacao)} />
            )}
            {creditos.material && (
              <ChipKV fieldId="detalhes_creditos_material" k="Material" v={creditos.material} />
            )}
            {creditos.autorouinstituicao && (
              <ChipKV
                fieldId="detalhes_creditos_autor"
                k="Autor/Instituição"
                v={creditos.autorouinstituicao}
              />
            )}
            {creditos.ano && <ChipKV fieldId="detalhes_creditos_ano" k="Ano" v={creditos.ano} />}
            <ChipKV
              fieldId="detalhes_creditos_generico"
              k="Precisa crédito genérico"
              v={creditos.precisacreditogenerico ? 'Sim' : 'Não'}
              cls={creditos.precisacreditogenerico ? 'coerencia-chip--bad' : 'coerencia-chip--ok'}
            />
          </div>
          ...
          {(() => {
            ...
            if (creditos.comoidentificou) {
              if (isReviewMode && onApprove && onReject) {
                return (
                  <div
                    className={`coerencia-obs ${evidenciaStateClass}`}
                    style={{ padding: '8px', borderRadius: '6px' }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                      <div className="coerencia-obs-title" style={{ flex: 1 }}>
                        Evidência
                      </div>
                      <div className="review-btn-group">
                        <button
                    ...
                    <div>
                      <SafeText text={creditos.comoidentificou} />
                    </div>
                  </div>
                );
              }
            ...
            if (isReviewMode && onApprove && onReject) {
              return (
                <div
                  className={`coerencia-obs coerencia-obs--empty ${evidenciaStateClass}`}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px',
          ...
          {creditos.textocreditosugerido && (
            <div className="coerencia-obs" style={{ marginTop: '8px' }}>
              ...
              <div>
                <SafeText text={creditos.textocreditosugerido} />
              </div>
            </div>
          )}
        </div>
          ...
          <div className="data-box scrollable">
            <ul>
              {alertasCredito.map((a, i) => {
                ...
                const alertaStateClass =
                  alertaState === 'approved'
                    ? 'block-approved'
                    : alertaState === 'rejected'
                      ? 'block-rejected'
                      : '';
                ...
                if (isReviewMode && onApprove && onReject) {
                  return (
                    <li
                      key={i}
                      className={`obs-item ${alertaStateClass}`}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                    ...
                    >
                      ...
                      <div className="review-btn-group">
                        <button
                          type="button"
                          className={`review-btn review-btn--approve review-btn--xs ${alertaState === 'approved' ? 'active' : ''}`}
                          onClick={() => onApprove(alertaFieldId)}
                        >
                          ✓
                        </button>
                        <button
                          type="button"
                          className={`review-btn review-btn--reject review-btn--xs ${alertaState === 'rejected' ? 'active' : ''}`}
                          onClick={() => onReject(alertaFieldId)}
                        >
                          ✗
                        </button>
                      </div>
...
const ReviewableItem: React.FC<{
  ...
  if (inline) {
    return (
      <div
        className={`reviewable-item-inline ${stateClass}`}
        style={{
          display: 'flex',
          alignItems: 'center',
          gap: '8px',
      ...
      >
        {label && (
          <span
            style={{ fontSize: '10px', color: 'var(--color-text-secondary)', marginRight: '4px' }}
          >
            {label}
          </span>
        )}
  ...
  return (
    <div
      ...
      style={{ margin: '8px 0', padding: '10px', borderRadius: '8px' }}
    >
      <div
        className="reviewable-block-header"
...
const ComplexityCardReviewable: React.FC<{
  ...
  const justStateClass =
    justState === 'approved' ? 'block-approved' : justState === 'rejected' ? 'block-rejected' : '';
  ...
  return (
    ...
    >
      ...
      {isReviewMode && onApprove && onReject ? (
        <div
          className={`reviewable-block ${nivelStateClass}`}
          style={{ padding: '10px', marginBottom: '10px', borderRadius: '8px' }}
        >
          <div
            style={{
              display: 'flex',
      ...
      {data.justificativa_dificuldade &&
        (isReviewMode && onApprove && onReject ? (
          <div
            className={`reviewable-block ${justStateClass}`}
            style={{ padding: '10px', marginBottom: '10px', borderRadius: '8px' }}
          >
      ...
      <details
        style={{ fontSize: 12, borderTop: '1px solid var(--color-border)', paddingTop: 8 }}
        open={isReviewMode}
      >
        <summary
          style={{
...
export const GabaritoCardView: React.FC<{ dados: GabaritoData } & GabaritoReviewProps> = ({
  dados,
  isReviewMode,
  reviewState,
  onApprove,
  ...
  return (
    <>
        ...
        <div className="questionText gabarito-head">
          ...
          {!isDissertativa &&
            (isReviewMode && onApprove && onReject ? (
              ...
              >
                <p style={{ margin: 0 }}>
                  <strong>Alternativa correta:</strong> <SafeText text={respostaLetra} />
                </p>
              </ReviewableItem>
            ) : (
              <p>
          ...
          {isDissertativa && (
            ...
            >
              <p style={{ margin: '0 0 10px 0', fontSize: '13px', color: 'var(--color-text)' }}>
                Esta questão será corrigida baseada nos seguintes{' '}
                <strong>Fatores e Critérios</strong> esperados pela IA:
              </p>
              ...
              {dados.respostaModelo &&
                (isReviewMode && onApprove && onReject ? (
                  ...
                  >
                    ...
                    >
                      <SafeMarkdown text={dados.respostaModelo} />
                    </div>
                  </ReviewableItem>
                ) : (
              ...
              {justificativaCurta &&
                (isReviewMode && onApprove && onReject ? (
                  ...
                  >
                    ...
                    >
                      <SafeMarkdown text={justificativaCurta} />
                    </div>
                  </ReviewableItem>
                ) : (
                  <div style={{ marginBottom: '12px' }}>
              ...
              {!dados.respostaModelo &&
                ...
                !(dados.questao?.palavras_chave?.length > 0) && (
                  <span style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>
                    Nenhum critério específico extraído ainda.
                  </span>
                )}
            </div>
          )}
          ...
          {!isDissertativa &&
            ...
            (isReviewMode && onApprove && onReject ? (
              <ReviewableItem
                ...
                label="📝 Justificativa"
              >
                <div className="gabarito-just markdown-content">
                  <SafeMarkdown text={justificativaCurta} />
                </div>
              </ReviewableItem>
            ) : (
      ...
      {(() => {
        ...
        const relatorioStateClass =
          relatorioState === 'approved'
            ? 'block-approved'
            : relatorioState === 'rejected'
              ? 'block-rejected'
              : '';
        ...
        if (isReviewMode && onApprove && onReject) {
          return (
            <div
              className={`reviewable-block ${relatorioStateClass}`}
              style={{ margin: '15px 0', padding: '10px', borderRadius: '8px' }}
            >
              <div
                className="reviewable-block-header"
                style={{ display: 'flex', alignItems: 'center', marginBottom: '6px' }}
              >
                ...
                <div className="review-btn-group" style={{ marginLeft: 'auto' }}>
                  <button
                    type="button"
                    className={`review-btn review-btn--approve review-btn--xs ${relatorioState === 'approved' ? 'active' : ''}`}
                    onClick={() => onApprove(relatorioFieldId)}
                    title="Aprovar relatório técnico"
                  >
                    ✓
                  </button>
                  <button
                    type="button"
                    className={`review-btn review-btn--reject review-btn--xs ${relatorioState === 'rejected' ? 'active' : ''}`}
                    onClick={() => onReject(relatorioFieldId)}
                    title="Rejeitar relatório técnico"
                  >
                    ✗
                  </button>
                </div>
              ...
              >
                {dados.textoReferencia ? (
                  <SafeText text={dados.textoReferencia} />
                ) : (
                  <em>Relatório de pesquisa não disponível.</em>
                )}
              </div>
            </div>
          );
        ...
        return (
          <details className="gabarito-extra" open={!dados.textoReferencia}>
            <summary>📄 Relatório Técnico</summary>
            <div
              className="markdown-content relatorio-content"
              data-raw={dados.textoReferencia || ''}
              style={{
...
export const AcoesGabaritoView: React.FC<{ onEdit: () => void; onFinish: () => void }> = ({
  onEdit,
  onFinish,
}) => {
  return (
    <div className="result-actions" id="actionsLeituraGabarito" style={{ marginTop: '15px' }}>
      ...
      <button
        ...
        id="btnFinalizarTudo"
        onClick={onFinish}
        style={{ marginTop: '10px', fontWeight: 'bold', border: '1px solid rgba(0,0,0,0.1)' }}
      >
        ✨ Finalizar Questão
      </button>
    </div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <button
                  type="button"
                  className="btn btn--sm btn--outline btn-remove-step"
                  style={{
              ...
              <div className="structure-editor-wrapper">
                <div
                  className="structure-editor-container step-drag-container"
                  style={{ minHeight: '50px', background: 'var(--color-background)' }}
                >
                  <RawHTML html={blocosHtml} />
                </div>
                <div
                  className="structure-toolbar step-add-toolbar"
                  style={{
                    marginTop: '8px',
                    paddingTop: '8px',
                    borderTop: '1px dashed var(--color-border)',
                ...
                >
                  <button
                    type="button"
                    className="btn btn--sm btn--secondary btn--full-width btn-toggle-step-add"
                    style={{
                      display: 'flex',
                  ...
                  <div
                    className="step-menu-content hidden"
                    style={{
                      position: 'absolute',
                      top: '100%',
                      left: 0,
                  ...
                  >
                    {[
                      ...
                      <button
                        key={type}
                        type="button"
                        className="btn btn--sm btn--outline btn-add-step-item"
                        data-type={type}
                      >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <select
                    className="form-control passo-origem"
                    style={{ width: '100%' }}
                    defaultValue={isExtraido ? 'extraido_do_material' : 'gerado_pela_ia'}
                  >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    className="form-control passo-fonte"
                    placeholder="Ex: Página 32..."
                    defaultValue={safe(p.fontematerial || '')}
                    style={{ width: '100%' }}
                  />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  >
                    Evidência Visual (se houver)
                  </span>
                  <input
                    className="form-control passo-evidencia"
                    placeholder="Ex: Gráfico azul, segundo parágrafo..."
                    defaultValue={safe(p.evidencia || '')}
                    style={{ width: '100%' }}
                  />
                </div>
              </div>
...
const EditorComplexidade: React.FC<{ complexidadeRaw: any }> = ({ complexidadeRaw }) => {
  ...
  return (
    ...
    >
      <span
        className="field-label"
        style={{ color: 'var(--color-primary)', marginBottom: '8px', display: 'block' }}
      >
        Matriz de Complexidade
      </span>
      <div style={{ fontSize: '11px', color: 'gray', marginBottom: '10px' }}>
        Marque os fatores determinantes para a dificuldade.
      </div>
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        <textarea
          id="editComplexidadeJust"
          className="form-control"
          rows={2}
          placeholder="Explique por que é difícil..."
          defaultValue={safe(complexidadeRaw?.justificativa_dificuldade || '')}
        ></textarea>
      </div>
    </div>
  );
...
export const GabaritoEditorView: React.FC<{
  dados: GabaritoData;
  onSave: () => void;
  onCancel: () => void;
}> = ({ dados, onSave, onCancel }) => {
  const {
    respostaLetra,
    justificativaCurta,
    confianca,
    ...
    <form id="gabaritoEdit">
      {!isDissertativa && (
        <div className="field-group">
          ...
          <input
            id="editGabaritoResposta"
            className="form-control"
            type="text"
            defaultValue={safe(respostaLetra)}
            placeholder="Ex.: A"
          />
        </div>
      )}
      {isDissertativa && (
        <div className="field-group">
          ...
          <textarea
            id="editGabaritoRespostaModelo"
            className="form-control"
            rows={4}
            placeholder="Digite a resposta ou critério principal esperado..."
            defaultValue={safe(dados.respostaModelo)}
          ></textarea>
        </div>
      )}
      <div className="field-group">
        ...
        <textarea
          ...
          defaultValue={safe(justificativaCurta || '')}
        ></textarea>
      </div>
      <div className="field-group">
        <span className="field-label">Confiança (0–1)</span>
        <input
          id="editGabaritoConfianca"
          className="form-control"
          type="number"
          min={0}
          max={1}
      ...
      {!isDissertativa && (
        <div className="field-group">
          ...
          <div id="editGabaritoAnalises" className="alts-list">
            {(Array.isArray(questao?.alternativas) ? questao.alternativas : []).map(
              (alt: any, i: number) => {
                ...
                return (
                  <div
                    key={i}
                    className="alt-row alt-edit-row"
                    style={{
                      display: 'flex',
                      gap: '6px',
                  ...
                  >
                    ...
                    />
                    <textarea
                      className="form-control gabarito-motivo"
                      data-letra={safe(letra)}
                      rows={2}
                      placeholder="Motivo (correta/errada)"
                      defaultValue={safe(analise?.motivo || '')}
                    ></textarea>
                  </div>
                );
              },
      ...
      <div className="field-group">
        ...
        <textarea
          ...
          className="form-control"
          rows={3}
          placeholder="Observações de consistência"
          defaultValue={safe(
            (Array.isArray(coerencia?.observacoes) ? coerencia.observacoes : []).join('\n'),
          )}
        ></textarea>
      </div>
      ...
      <div className="field-group">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ flex: 1, minWidth: '160px' }}>
            ...
            <input
              ...
              className="form-control"
              type="text"
              defaultValue={safe(creditos.origemresolucao || '')}
          ...
          <div style={{ flex: 1, minWidth: '160px' }}>
            ...
            <input
              ...
              className="form-control"
              type="text"
              defaultValue={safe(creditos.material || '')}
          ...
          <div style={{ flex: 1, minWidth: '160px' }}>
            ...
            <input
              ...
              className="form-control"
              type="text"
              defaultValue={safe(creditos.autorouinstituicao || '')}
          ...
          <div style={{ flex: '0 0 100px' }}>
            ...
            <input
              ...
              className="form-control"
              type="text"
              defaultValue={safe(creditos.ano || '')}
              placeholder="2024"
            />
          </div>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginTop: '8px' }}>
          <div style={{ flex: '0 0 140px' }}>
            ...
            <label style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '4px' }}>
              <input
                id="editCredMatIdentificado"
                type="checkbox"
                defaultChecked={creditos.materialidentificado}
              />
          ...
          <div style={{ flex: '0 0 170px' }}>
            ...
            <label style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '4px' }}>
              <input
                id="editCredPrecisaGenerico"
                type="checkbox"
                defaultChecked={creditos.precisacreditogenerico}
              />
              <span style={{ fontSize: '12px' }}>Sim</span>
            </label>
          </div>
          <div style={{ flex: 1, minWidth: '160px' }}>
            ...
            <input
              ...
              max={1}
              step={0.01}
              defaultValue={creditos.confiancaidentificacao ?? ''}
            />
          </div>
        </div>
        <div style={{ marginTop: '8px' }}>
          ...
          <textarea
            ...
            className="form-control"
            rows={2}
            placeholder="Cabeçalho, rodapé, diagramação..."
            defaultValue={safe(creditos.comoidentificou || '')}
          ></textarea>
        </div>
        <div style={{ marginTop: '8px' }}>
          <span className="field-label">Crédito sugerido (texto)</span>
          <textarea
            id="editCredTextoSugerido"
            className="form-control"
            rows={2}
            placeholder="Texto pronto para mostrar como crédito."
            defaultValue={safe(creditos.textocreditosugerido || '')}
          ></textarea>
        </div>
      </div>
      ...
      <div className="field-group">
        ...
        <textarea
          id="editGabaritoAlertas"
          className="form-control"
          rows={3}
          defaultValue={safe(alertasCredito.join('\n'))}
        ></textarea>
      </div>
      <div className="field-group">
        ...
        <textarea
          ...
          className="form-control"
          rows={3}
          defaultValue={safe(observacoes.join('\n'))}
        ></textarea>
      </div>
