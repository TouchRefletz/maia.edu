...
const ChatContentBlockRenderer: React.FC<{
  ...
  switch (tipo) {
    ...
    case 'equacao':
      return <div className={`chat-block chat-equacao ${className}`}>{`\\[${conteudoRaw}\\]`}</div>;
    case 'codigo': {
    ...
    case 'separador':
      return <div className={`chat-block chat-separador ${className}`} />;
    case 'imagem':
...
export const generateChatHtmlString = (data: ChatResponse | any): string => {
  ...
  return ReactDOMServer.renderToStaticMarkup(<ChatLayoutRenderer data={data} />);
};
