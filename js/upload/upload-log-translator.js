...
export class UploadLogTranslator {
  static translate(message) {
    ...
    if (msg.includes('Clone Hugging Face Repo') || msg.includes('Baixando base de dados'))
      return { percent: 75, text: 'Sincronizando com repositório global...' };

    if (msg.includes('Download Files') || msg.includes('Baixando arquivos processados'))
    ...
    if (msg.includes('Compute Visual Hashes') || msg.includes('Validando integridade visual'))
      return {
        percent: 85,
        text: 'Revalidando integridade dos dados na nuvem...',
      };

    if (msg.includes('Generate Thumbnails') || msg.includes('Generating thumbnails'))
      return { percent: 90, text: 'Gerando pré-visualizações e miniaturas...' };

    if (msg.includes('Push to Hugging Face') || msg.includes('Salvando alterações'))
    ...
    // --- COMPLETION ---
    if (msg.includes('Cloud sync complete') || msg.includes('Files and Thumbnails are live')) {
