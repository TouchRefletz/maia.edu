...
export function setupFormLogic(elements, initialData) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const executeUploadSequence = async () => {
      ...
      `;
        ...
        const interval = setInterval(() => {
          if (currentProgress < targetProgress) {
            ...
            if (Math.abs(targetProgress - currentProgress) < 0.5) currentProgress = targetProgress;
          }
        ...
        const addLog = (text, isSystem = false) => {
          ...
          line.style.color = isSystem ? 'var(--color-primary, #569cd6)' : 'inherit';
      ...
      const progress = showProgressModal('Calculando identidade visual do arquivo...', () => {
        const confirmModal = document.getElementById('cancelUploadConfirmModal');
        ...
        if (confirmModal) {
          ...
          btnConfirm.onclick = () => {
            ...
            progress.addLog('Cancelando nuvem... Abrindo visualização local.', true);
      ...
      const openViewer = (hfUrl, slug, aiData, hfUrlGabarito) => {
        ...
        gerarVisualizadorPDF({
          ...
          rawTitle: aiData?.formatted_title_general || userTitle || fileProva.name,
          fileProva: proxyUrl,
      ...
      const startPollingAndOpenViewer = async (hfUrl, slug, aiData, hfUrlGabarito) => {
        ...
        if (!PusherClass) {
          try {
            ...
            customAlert('Erro ao carregar sistema de notificação em tempo real.');
            return;
          }
        }
        ...
        const safetyTimeout = setTimeout(() => {
          if (!eventReceived) {
            progress.addLog('⚠️ O processo está demorando mais que o normal ou não iniciou.', true);
        ...
        channel.bind('log', (data) => {
          ...
          if (msg && msg.includes('Cloud sync complete')) {
            progress.addLog('Sincronização concluída! Abrindo visualizador...', true);
      ...
      try {
        ...
        if (linkGab) {
          ...
          if (blob) {
            progress.addLog('Download via link com sucesso! Enviando para TmpFiles...');
            try {
              tmpUrlGabLink = await uploadToTmpFiles(blob, 'gabarito_link_temp.pdf', signal);
              console.log('[Manual] TmpUrl Gabarito (Link):', tmpUrlGabLink);
              progress.addLog('✅ Link do gabarito processado.');
            } catch (err) {
              console.warn('[Manual] Upload TmpFiles (Link Gabarito) Failed:', err);
            }
          }
        }
        // --------------------------------------
        ...
        const WORKER_URL = 'https://maia-api-worker.willian-campos-ismart.workers.dev';
        ...
        const getRemoteHash = async (url, tempSlug, label) => {
          ...
          if (!response.ok) {
            ...
            throw new Error(`Worker Error (${response.status}): ${errorMessage}`);
          }
        ...
        if (hashTasks.length > 0) {
          ...
          progress.addLog(`Iniciando cálculo de hash para ${hashTasks.length} itens...`);
          ...
          for (const outcome of results) {
            ...
            if (outcome.status === 'fulfilled') {
              ...
              const displayHash = validHash ? validHash.substring(0, 15) + '...' : '(VAZIO/NULL)';
          ...
          try {
            ...
            const preflightRes = await fetch(`${WORKER_URL}/trigger-deep-search`, {
              method: 'POST',
            ...
            if (predictedSlug) {
              ...
              if (manifestRes.ok) {
                ...
                console.log('[Manual] Manifesto encontrado. Comparando hashes...', manifestItems);
                progress.addLog(`Pasta '${predictedSlug}' encontrada. Comparando arquivos...`);
                ...
                const matchProva = remoteHashProva
                  ...
                  : null;
                const matchGab = remoteHashGab ? findInManifest(remoteHashGab, 'gabarito') : null;
                ...
                if (matchProva) {
                  ...
                  progress.addLog('✅ Prova já existe na nuvem (Hash idêntico).');
                  uploadProva = false;
                ...
                if (firstItem) {
                  aiDataFromManifest = {
                    ...
                    formatted_title_general:
                      firstItem.nome || firstItem.friendly_name || firstItem.name,
                    // Captura source_url_prova para sistema de imagens via PDF
                    source_url_prova:
                      firstItem.source_url_prova ||
                      manifestItems.find((i) => i.source_url_prova)?.source_url_prova ||
                      null,
                  };
                }
              } else {
                console.log('[Manual] Manifesto não encontrado (404). Upload completo necessário.');
              }
            }
          } catch (eChecks) {
            ...
            if (uploadGabarito) {
              if (!tmpUrlGabLink) {
                ...
                if (remoteHashGab !== remoteHashGabLink) {
                  progress.addLog('❌ BLOQUEIO (Gabarito): Hash do arquivo difere do Link.', true);
                  uploadGabarito = false;
                } else {
                  progress.addLog('✅ Gabarito validado (Hash Link == Hash Arquivo).');
                }
              } else {
                if (!remoteHashGabLink) {
                  progress.addLog('⚠️ Link do gabarito não processado. Cancelando upload.');
                  uploadGabarito = false;
                }
              }
            }
          ...
          const checkCopyright = async (file, typeLabel) => {
            ...
            try {
              ...
              if (!response.ok) throw new Error('Erro na verificação de copyright.');
              ...
              if (finalResult) {
                try {
                  ...
                  if (json.protected) {
                    progress.addLog(`❌ BLOQUEIO COPYRIGHT (${typeLabel}): ${json.reason}`, true);
              ...
              progress.addLog(`⚠️ Erro ao verificar copyright: ${e.message}. Permitindo...`);
              return true; // Fail open
            }
          };
          ...
          if (skipUpload) {
            console.log('[Manual] ALL FILES EXIST IN CLOUD. Skipping upload flow completely.');
            progress.setTarget(100, 'Concluído');
            progress.addLog('Todos os arquivos já existem. Abrindo visualizador local...');

            setTimeout(() => {
              progress.close();

              // Capturar source_url_prova do manifesto existente se disponível
              const manifestProvaUrl = aiDataFromManifest?.source_url_prova || linkProva || null;
              if (manifestProvaUrl) {
              ...
              gerarVisualizadorPDF({
                title: aiDataFromManifest?.formatted_title_general || userTitle || fileProva.name,
                rawTitle:
                  aiDataFromManifest?.formatted_title_general || userTitle || fileProva.name,
        ...
        const srcProvaVal = document.getElementById('sourceUrlProva')?.value.trim() || '';
        const srcGabVal = document.getElementById('sourceUrlGabarito')?.value.trim() || '';

        // --- NEW NAMING LOGIC START ---
        // --- NEW NAMING LOGIC START ---
        const getSafeName = (fileInputName, type, fallbackTitle) => {
          const forbidden = ['prova.pdf', 'gabarito.pdf', 'nenhum arquivo selecionado', ''];
        ...
        if (uploadProva) {
          ...
          if (hashP) {
            ...
            progress.addLog(`Anexando visual_hash (Prova): ${hashP.substring(0, 10)}...`);
          } else {
        ...
        if (uploadGabarito) {
          ...
          if (hashG) {
            ...
            progress.addLog(`Anexando visual_hash (Gabarito): ${hashG.substring(0, 10)}...`);
          }
        ...
        try {
          for (const [key, value] of formData.entries()) {
            if (value instanceof File) console.log(`[Manual] Key: ${key} -> File: ${value.name}`);
        ...
        const WORKER_URL_UPLOAD = 'https://maia-api-worker.willian-campos-ismart.workers.dev';
        ...
        if (data.status === 'conflict') {
          progress.addLog('⚠️ Aviso: Arquivos similares detectados no servidor.');
        ...
        if (uploadProva || uploadGabarito) {
          progress.addLog('Aguardando confirmação de processamento (Sync)...', true);
          ...
          await new Promise(async (resolve, reject) => {
            ...
            setTimeout(() => {
              ...
              progress.addLog('⚠️ Tempo limite de sync atingido. Abrindo assim mesmo...');
              safeResolve();
            }, 15000); // 15s max wait
            ...
            channel.bind('log', (d) => {
              ...
              if (msg.includes('Cloud sync complete') || msg.includes('Processamento concluído')) {
        ...
        setTimeout(() => {
          ...
          gerarVisualizadorPDF({
            ...
            originalPdfUrl:
              ...
              linkProva ||
              data.hf_url_preview ||
              aiDataFromManifest?.source_url_prova ||
              null, // Garante que o contexto receba a URL oficial
          });
        }, 800);
        ...
        const isAbort =
          e.name === 'AbortError' ||
          (e.message && (e.message.includes('AbortError') || e.message.includes('Cancelado'))) ||
          signal.aborted;
    ...
    const showProcessingConfirmation = () => {
      ...
      const checkboxContainer = document.getElementById('copyrightCheckContainer');
