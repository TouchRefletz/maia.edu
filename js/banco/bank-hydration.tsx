...
export function hydrateBankCard(
  ...
  data: { q: any; g: any; imgsOriginalQ: string[]; jsonImgsG: string },
) {
    ...
    data.g.explicacao.forEach((passo: any, idx: number) => {
      ...
      if (stepContainer) {
        ...
        const estrutura = Array.isArray(passo.estrutura)
          ? passo.estrutura
          : [{ tipo: 'texto', conteudo: passo.passo || '' }];
  ...
  if (data.q && Array.isArray(data.q.alternativas)) {
    data.q.alternativas.forEach((alt: any) => {
      const letra = String(alt?.letra || '')
        ...
        .toUpperCase();
      if (!letra) return;
      const optContent = cardElement.querySelector(
        `.q-opt-btn[data-letra="${letra}"] .q-opt-content`,
      );
