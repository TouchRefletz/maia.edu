...
export const uploadToImgBB = async (base64String) => {
  console.warn('[payload-imagens] uploadToImgBB foi deprecado. Imagens são renderizadas via PDF.');
  return null;
};
...
export const processarObjetoRecursivo = async (obj, btnEnviar) => {
  ...
  if (Array.isArray(obj)) {
    for (let i = 0; i < obj.length; i++) {
      ...
      if (typeof val === 'string' && val.startsWith('blob:')) {
        console.log(`[payload-imagens] Limpando blob URL temporário no array[${i}]`);
        obj[i] = null;
      }
  ...
  for (const key in obj) {
    ...
    // Se temos dados de crop, podemos limpar imagem_url que é blob
      ...
      console.log(`[payload-imagens] Limpando blob imagem_url (temos dados de crop)`);
      obj[key] = null;
    }
